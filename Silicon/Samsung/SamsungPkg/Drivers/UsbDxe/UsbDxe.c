  ...
  )
{
  ...
  Dwc3Reg = (DWC3 *)(UsbReg + DWC3_REG_OFFSET);

  MmioAndThenOr32 ((UINTN)&Dwc3Reg->GSBusCfg0, ~USB2_ENABLE_BEAT_BURST_MASK, USB2_ENABLE_BEAT_BURST);
...
{
  MmioOr32 ((UINTN)&Dwc3Reg->GFLAdj, GFLADJ_30MHZ_REG_SEL | GFLADJ_30MHZ(Val));
}
...
{
  // Put the core in reset before resetting PHY
  ...
  // Take the core out of reset, PHYs are stable now
  ...
  )
{
  ...
  // This should read as 0x5533, ascii of U3 (DWC_usb2) followed by the revision num
...
/**
  This function gets registered as a callback to perform USB controller initialization
...
)
{
  ...
  for (Index = 0; Index < NumUsbController; Index++) {
    ControllerAddr = PcdGet64(PcdUsbBaseAddr) + (Index * PcdGet32(PcdUsbSize));
...
/**
  The Entry Point of the module. It follows the standard UEFI driver model.
